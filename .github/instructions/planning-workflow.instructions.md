...
- Ensures GitHub discussion stays synchronised with plan document
...
**What it does:** Creates test specifications for all implementation steps. Tests come before implementation code.
...
**What it does:** Writes detailed implementation code blocks with proper structure. For complex implementations, uses scratch files.
...
**What it does:** Implements the actual code based on the completed plan document. This is where you write the real production code.
...
2c. "Post answers to GitHub for issue #61"
    → Synchronises answers back to GitHub issue
...
- **Steps 1-7:** Work on plan documents only (e.g., `features/*.md` or `plans/*.md`)
- **Step 8:** Implements actual code (e.g., `src/`, `tests/`, `lib/`)
- **Step 7 is mandatory:** Always verify plan before implementation
- **Scratch files:** Temporary working files for complex implementations, delete before merge
...
- **Test standards:** Language-specific instruction files
