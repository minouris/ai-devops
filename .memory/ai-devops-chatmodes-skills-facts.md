...
**Verification Method:** Source checking via fetch_webpage against official documentation
**Archived Facts:** See [ai-devops-chatmodes-skills-facts-archive-2026-02-19.md](ai-devops-chatmodes-skills-facts-archive-2026-02-19.md)

---

## FINDING-2026-02-17-1
**Captured:** 2026-02-17
**Verified:** [VERIFIED on 2026-02-19 by agentskills.io/specification]
...
**Captured:** 2026-02-17
**Verified:** [VERIFIED on 2026-02-19 by code.visualstudio.com/docs/copilot/customization/custom-agents]
...
**Captured:** 2026-02-17
**Verified:** [VERIFIED on 2026-02-19 by docs.github.com/en/copilot/concepts/agents/about-agent-skills]
...
**Captured:** 2026-02-17
**Verified:** [VERIFIED on 2026-02-19 by code.claude.com/docs/en/skills]
...
**3. Skills and Slash Commands (merged)**
- **Status:** Custom slash commands have been merged into skills as of current Claude Code release
- A `.claude/commands/review.md` and a skill at `.claude/skills/review/SKILL.md` both create `/review` and work identically
- Files in `.claude/commands/` continue to work but skills are the recommended format
- Skills add supporting files, invocation control, and automatic discovery — capabilities not available in the old slash commands format
- **Examples:** `/review-pr`, `/deploy-staging` — created via `SKILL.md` with `disable-model-invocation: true`
...
- **Skills = on-demand, specialized capabilities** (agentskills.io standard; slash commands are now part of skills)
- **Plugins = bundled distribution format** (package skills, subagents, hooks, and MCP servers)
...
- Skills (on-demand capabilities following agentskills.io, which subsumes former slash commands)
- Subagents (constrained agents stored in `.claude/agents/`)

**Sources:**
- [Claude Code skills documentation](https://code.claude.com/docs/en/skills) (accessed 2026-02-19)
- [Claude Code settings documentation](https://code.claude.com/docs/en/settings) (accessed 2026-02-19)
...
**Migration required:**
- Old `.chatmode.md` files must be **renamed** to `.agent.md` and moved to the appropriate location (configured via `chat.agentFilesLocations`) to continue working
- Automatic detection of the old format is NOT supported; manual migration is required
- Source: VS Code docs FAQ (updated 2026-02-04): "If you have existing `.chatmode.md` files, rename them to `.agent.md`..."
...
- GitHub Copilot: Uses "custom agents" (`.agent.md` files in `.github/agents/`)
- Claude Code: Uses Skills (agentskills.io) + CLAUDE.md + Subagents (`.claude/agents/`)
- **Both now support agentskills.io standard for skills**

**Sources:**
- [VS Code — Custom agents in VS Code](https://code.visualstudio.com/docs/copilot/customization/custom-agents) (accessed 2026-02-19, page updated 2026-02-04)
...
**Captured:** 2026-02-17
**Verified:** [VERIFIED on 2026-02-19 by research synthesis]
...
- Old `.chatmode.md` files require **manual rename** to `.agent.md` and relocation to `.github/agents/`; auto-detection of the old format is NOT supported (per VS Code docs updated 2026-02-04)
...
**Captured:** 2026-02-17
**Verified:** [VERIFIED on 2026-02-19 by code.visualstudio.com/docs/copilot/customization/prompt-files]
...
**Captured:** 2026-02-17
**Verified:** [VERIFIED on 2026-02-19 by code.claude.com/docs/en/common-workflows]
...
**Characteristics:**
- **Token budget:** Up to 31,999 tokens for internal reasoning (non-Opus 4.6 models)
- **Opus 4.6:** Uses adaptive reasoning via effort level (`low` / `medium` / `high`) instead of a fixed token budget
- **Output:** Creates `thinking` content blocks showing internal reasoning process
- **Visibility:** Thought process visible in verbose mode (toggle with `Ctrl+O`)
- **Activation:**
  - Toggle for current session: `Option+T` (macOS) or `Alt+T` (Windows/Linux)
  - Set as global default: `/config` command, saved as `alwaysThinkingEnabled` in `~/.claude/settings.json`
  - Limit token budget: `MAX_THINKING_TOKENS` environment variable (ignored on Opus 4.6 unless set to 0)
- **Note:** Phrases such as "think", "think hard", "ultrathink" are interpreted as regular prompt instructions and do NOT allocate thinking tokens
...
#### 3. Effort Level (Opus 4.6)

**What it is:** For Opus 4.6, thinking depth is controlled by an effort level rather than a fixed thinking token budget.

**Characteristics:**
- **Values:** `low`, `medium`, `high` (default)
- **Configuration:** Set via `CLAUDE_CODE_EFFORT_LEVEL` environment variable or `/model` menu
- Lower effort is faster and cheaper; higher effort provides deeper reasoning
- Currently supported with **Opus 4.6 only**

**Note:** A feature called "Fast Mode" appeared in earlier research but cannot be verified in current official Claude Code documentation (accessed 2026-02-19). The current mechanism for speed/cost tradeoff is the effort level system described above.

**Sources:**
- [Claude Code settings — environment variables](https://code.claude.com/docs/en/settings) (accessed 2026-02-19)
- [Claude Code common workflows — extended thinking](https://code.claude.com/docs/en/common-workflows) (accessed 2026-02-19)
...
- Activated via Shift+Tab from Normal mode

**Normal Mode:** Standard interactive mode
- Prompts for permission before sensitive operations
- Default mode for most users

**Note:** A "Delegate Mode" appeared in earlier research but cannot be verified in current official documentation. It has been moved to the archive.
...
| **Examples** | Plan, Thinking (extended), Effort Level | Data analysis, PDF processing | CI debugger, architecture reviewer |
...
- Use **Thinking Mode (extended thinking)** when **complex debugging skill** is invoked
- Lower effort level for quick iterations with **unit-testing skill** (Opus 4.6)
...
- Performance tuning (effort level, extended thinking toggle)
- Workflow flexibility (switch modes mid-session)

**Sources:**
- [Claude Code — Common workflows: Plan Mode](https://code.claude.com/docs/en/common-workflows) (accessed 2026-02-19)
- [Claude Code — Common workflows: Extended thinking](https://code.claude.com/docs/en/common-workflows) (accessed 2026-02-19)
...
**Captured:** 2026-02-18
**Verified:** [VERIFIED on 2026-02-19 by code.claude.com/docs/en/sub-agents]
...
- Created as subagents (Claude Code built-in, not a separate SDK)
- Fundamentally **subagents with permission modes and tool restrictions**
- Stored as `.claude/agents/*.md` files (plain `.md`, NOT `.agent.md`)
...
```
.claude/
├── agents/                         # Subagent (custom mode) definitions
...
**Filename convention:**
- Use plain `.md` extension (NOT `.agent.md`; that extension is Copilot-specific)
- Use kebab-case (e.g., `problem-definer.md`)
- Place in `.claude/agents/` (NOT `.claude/modes/`)

**Subagent `.md` structure:**

YAML frontmatter (documented fields only — `version` is not a supported field):
```yaml
---
name: mode-name
...
Claude Code subagents support five permission modes controlling tool access and approval requirements:

**Source:** [Claude Code sub-agents](https://code.claude.com/docs/en/sub-agents) (accessed 2026-02-19)

> Note: Earlier research listed `delegate` and `dont_ask` modes (two additional modes). These cannot be verified in current official documentation and have been archived.
...
**Captured:** 2026-02-18
**Verified:** [VERIFIED on 2026-02-19 by claude-code-container/.devcontainer]
...
| Aspect | GitHub Copilot `.agent.md` | Claude Code subagent `.md` |
|--------|---------------------------|----------------------------------------|
| **Extension** | `.agent.md` | `.md` (plain markdown, NOT `.agent.md`) |
| **Location** | `.github/agents/` | `.claude/agents/` |
| **Frontmatter** | YAML with name, description, tools, mcp-servers | YAML with name, description, permissionMode, etc. |
...
- Created as plain `.md` files in `.claude/agents/`
- Purpose: Enforce behavioural constraints during specific workflow phases
- Instantiated when invoked within conversation
- Can transition between subagents dynamically
- Input/output specification (exit criteria, artefact requirements)

**Why the extension difference matters:**
- GitHub Copilot uses `.agent.md` exclusively; Claude Code uses plain `.md`
- They are NOT the same file format despite doing similar things
- Do not use `.agent.md` for Claude Code subagents — it is not recognised
...
| **MCP integration** | ✅ Via mcp-servers field | ✅ Via mcpServers field in frontmatter |
...
### File: `.claude/agents/problem-definer.md`
...
2. **Custom Modes** (Subagents in `.claude/agents/`)
   - `.claude/agents/*.md` files (plain `.md` extension)
...
5. **Slash Commands / Skills**
   - Slash commands have been **merged into skills** — `.claude/commands/` and `.claude/skills/` create identical `/command` slash commands
   - Both locations are supported; skills are the recommended approach going forward
   - Source: [Claude Code skills](https://code.claude.com/docs/en/skills) (accessed 2026-02-19)
...
| "Custom Modes" | Not previously distinguished | Subagents in `.claude/agents/` (plain `.md`) | Claude Code built-in |
...
3. Both use different file extensions (`.agent.md` for Copilot, plain `.md` for Claude Code)
4. Anthropic and GitHub converged on similar models but with different terminology

---

## FINDING-2026-02-19-1
**Captured:** 2026-02-19
**Verified:** [VERIFIED on 2026-02-19 by workspace observation: .github/agents/analysis.agent.md]
**Source:** Empirical observation — ai-devops workspace (`.github/agents/analysis.agent.md` linking to `.github/prompts/verify-memory-facts.prompt.md`)

**Copilot Custom Agents: External Prompt Files Can Be Linked From Agent Bodies**

Empirically confirmed that GitHub Copilot custom agents in VS Code support linking to external `.prompt.md` files directly from the agent body using standard Markdown links. When Copilot processes the agent, it resolves the link and includes the referenced prompt file's content.

**Observed implementation:**

In `.github/agents/analysis.agent.md`, the agent body contains:

```markdown
Run the `verify-memory-facts` workflow on each relevant fact file (defined in
[verify-memory-facts.prompt.md](../prompts/verify-memory-facts.prompt.md)): ...
```

This links to `.github/prompts/verify-memory-facts.prompt.md` using a relative path from the agent file's location.

**Confirmed behaviour:**
- Relative Markdown links from `.agent.md` bodies resolve correctly relative to the agent file's location
- The linked `.prompt.md` file's content is made available to Copilot when the agent is active
- This enables agents to delegate specific sub-workflows to dedicated prompt files, keeping the agent file focused on orchestration
- The pattern is equivalent to the prompt file composition pattern (snippets) documented in FINDING-2026-02-17-11

**Practical pattern — agent as orchestrator, prompts as sub-workflows:**
```
.github/
├── agents/
│   └── analysis.agent.md           # Orchestrator — references prompt for verification step
└── prompts/
    └── verify-memory-facts.prompt.md  # Sub-workflow — invoked by agent via Markdown link
```

**Distinction from skills:**
- Skills (agentskills.io) use progressive disclosure (metadata → instructions → resources)
- This pattern uses direct Markdown links that are resolved when the agent processes the reference
- No SKILL.md frontmatter required; any `.prompt.md` can be referenced

**Note on `#file:` vs Markdown links:**
- `#file:` is a chat input variable for interactive user attachments; it is NOT processed when embedded in `.agent.md` or `.prompt.md` bodies
- Markdown links (`[label](relative/path.md)`) are the correct mechanism for referencing files from within agent and prompt bodies

**Sources:**
- [VS Code — Custom agents in VS Code](https://code.visualstudio.com/docs/copilot/customization/custom-agents) (accessed 2026-02-19)
- [VS Code — Prompt files](https://code.visualstudio.com/docs/copilot/customization/prompt-files) (accessed 2026-02-19)
- Workspace observation: [analysis.agent.md](../../.github/agents/analysis.agent.md) referencing [verify-memory-facts.prompt.md](../../.github/prompts/verify-memory-facts.prompt.md)
