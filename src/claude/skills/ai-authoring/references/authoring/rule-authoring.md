...
Guide creation of rule artifacts using the rule structure rules as the authoritative definition of required structure.

---

## Structure Rules Reference

Read `src/claude/rules/rule-structure.md` before proceeding. That file is the single source of truth for required rule structure. All MUST and MUST NOT requirements in that file apply during authoring.
...
Read `src/claude/rules/rule-structure.md` in full. Use its MUST requirements as your checklist for what to create.
...
When all files are created, work through the Compliance Verification section of `src/claude/rules/rule-structure.md` line by line. Report each item as pass or fail. Fix any failures before marking authoring complete.

---

## MUST

- Read `src/claude/rules/rule-structure.md` before starting authoring
